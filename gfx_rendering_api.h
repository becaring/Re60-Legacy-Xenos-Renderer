...
#include <stdint.h>
#include <stdbool.h>
#include <string.h> // memcpy, used by hash_pair_ff below
...
// XDK-era STL lacks std::hash<float>; hash the float bit patterns directly.
struct hash_pair_ff {
    size_t operator()(const std::pair<float, float> &p) const {
        uint32_t bits1, bits2;
        memcpy(&bits1, &p.first, sizeof(float));
        memcpy(&bits2, &p.second, sizeof(float));
        size_t hash1 = (size_t)bits1;
        size_t hash2 = (size_t)bits2;
